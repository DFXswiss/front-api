...
grep -Fq 'Those routes are **not** endpoints of this' "$repo_root/CONTRIBUTING.md" || fail "unknown_forward: CONTRIBUTING process bullet must say those routes are not endpoints of this process"
grep -Fq 'Forwarding an unknown' "$repo_root/CONTRIBUTING.md" || fail "unknown_forward: CONTRIBUTING process bullet must say forwarding an unknown route does not list it"
...
grep -Fq 'Those routes belong to the upstream HTTP backend, not to' "$repo_root/README.md" || fail "unknown_forward: README local-answers must say those routes belong to the upstream HTTP backend"
grep -Fq 'this process. This repository does not name them.' "$repo_root/README.md" || fail "unknown_forward: README local-answers must say those routes do not belong to this process"
...
grep -Fq 'not endpoints of this process' "$repo_root/REVIEW.md" || fail "unknown_forward: REVIEW must say unknown routes are not endpoints of this process"
grep -Fq 'belong to the upstream HTTP backend' "$repo_root/REVIEW.md" || fail "unknown_forward: REVIEW must say unknown routes belong to the upstream HTTP backend"
